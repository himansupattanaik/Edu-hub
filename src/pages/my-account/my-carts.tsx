...
const CartItem = ({ id, title, price, quantity, onRemove, onQuantityChange }: any) => {
  return (
    <tr>
      <td className="p-4 px-6 text-center whitespace-nowrap">{title}</td>
      <td className="p-4 px-6 text-center whitespace-nowrap">
        <div>
          <button onClick={() => onQuantityChange(id, quantity - 1)} className="px-2 py-0 shadow">-</button>
          <input
            type="text"
            name="qty"
            value={quantity}
      ...
      <td className="p-4 px-6 text-center whitespace-nowrap">${price * quantity}</td>
      <td className="p-4 px-6 text-center whitespace-nowrap">
        <button onClick={() => onRemove(id)} className="px-2 py-0 text-red-100 bg-red-600 rounded">
          x
        </button>
      </td>
    </tr>
    ...
    <PublicLayout>
      ...
      <div className="main-container p-8 mx-auto mt-12 bg-white">
        <div className="w-full overflow-x-auto">
          <div className="my-2">
            <h3 className="text-xl font-bold tracking-wider">Shopping Cart {totalQty} item</h3>
          </div>
          <table className="w-full shadow-inner">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td className="p-4 px-6 text-center whitespace-nowrap">
                  <button onClick={handleClearAll} className="px-4 py-1 text-red-600 bg-red-100">
                    Clear All
                  </button>
                </td>
              </tr>
          ...
          <div className="flex justify-end mt-4 space-x-2">
            ...
            <button
              className="px-6 py-3 text-sm text-white bg-indigo-500 hover:bg-indigo-600"
            >
              Proceed to Checkout
            </button>
          </div>
